...
module Redmine::MenuManager::TopMenu::HelpMenu
  ...
  def render_help_and_support(result)
    ...
    result << static_link_item(:raise_documentation_issue)
    result << static_link_item(:request_feature)
    result << static_link_item(:raise_bug)
    result << static_link_item(:excel_download)
